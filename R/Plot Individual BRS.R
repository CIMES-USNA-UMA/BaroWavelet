...
PlotBRS <- function(fun, avg = FALSE, time_col = "brown", HFcolor = "yellow",
                    ...
                    use.thr = TRUE, nfreqs = 7, tem = FALSE, newPlot = TRUE, title = "data", plotHF = FALSE,
                    plotLF = TRUE){
  #if(dev.cur() > 1) 
  if(newPlot & !tem){
  ...
  if(fun$type == "brs_dwt"){
      im <- PlotDwtBRS(fun, time_flags = time_flags, col = time_col, tem = tem, plotHF = plotHF, plotLF = plotLF)
      return(im)
