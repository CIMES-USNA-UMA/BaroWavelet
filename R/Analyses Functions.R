...
#' @param framework An analysis environment generated by \link[BaroWavelet]{BuildStructure}
#' @param type Type of locator to be shown
#'
...
#' @examples
#' Study <- BuildStructure()
#' ShowIndexes(Study, "analyses")
#' ShowIndexes(Study, "controls")
#' ShowIndexes(Study, "intervals")
#' ShowIndexes(Study, "tests")
...
#' @param framework An analysis environment generated by \link[BaroWavelet]{BuildStructure}
#' @param a_locator An locator indicating which analysis slot needs to be used
#' @param t_locator An locator indicating which time inerval is to be used
#' @param time_flags A vector containing the minimum and maximum limits of a time interval, in minutes.
#'                   Default is NULL.
...
AnalyzeBRSIndices <- function(framework, a_locator, t_locator, time_flags,
   method = c("both", "dwt", "cwt"), use.name = TRUE){
                   method <- match.arg(method)
                   if(method == "both"){
                      framework <- AddIndividualIndices(framework, a_locator, t_locator, time_flags,
                         method = "dwt", use.name)
                      framework <- AddIndividualIndices(framework, a_locator, t_locator, time_flags,
                         method = "cwt", use.name)
                   } else {
                   ...
                   #framework <- AddTimeValues(framework, a_locator, t_locator, time_flags,
                   #    use.name)
                   if(method != "cwt") framework <- AddExpectedHRV(framework, a_locator, t_locator)
                   return(framework)
}
...
#' for(n in 1:4) Study <- GetAvgCwtData(Study, 1)
#' 
#' Study <- AddTimeInterval(Study, "First interval")
#' Study <- AddTimeInterval(Study, "Second interval")
#' 
...
AddIndividualIndices <- function(framework, a_locator, t_locator, time_flags,
   method = c("dwt", "cwt"), use.name = TRUE){
                   ...
                   if(method == "dwt"){
                          ...
                          tf$type <- "brs_dwt"
                          others <- interval$DWT
                          Time <- framework$IndividualIndices[[t_locator]]$Time_DWT
                          N <- length(others)
                   } else if(method == "cwt"){
                          tf <- AssembleCwtTransferFun(framework, a_locator)
                          tf$type <- "brs_cwt"
                          others <- interval$CWT
                          Time <- framework$IndividualIndices[[t_locator]]$Time_CWT
                          N <- length(others)
                   }
                   ...
                   } else if(a_locator <= ncol(others)){
                     results <- others
                     results[,a_locator] <- indices
                     Time[,a_locator] <- AdjustTime(tf, time_flags)
                   } else {
                   ...
                   if(use.name & !is.null(Analysis$Name)){
                         colnames(results)[a_locator] <- Analysis$Name
                   } else {
                         colnames(results)[a_locator] <- Analysis$locator
                   }
                   if(method == "dwt"){
                          framework$IndividualIndices[[t_locator]]$DWT <- results
                          framework$IndividualIndices[[t_locator]]$Time_DWT <- Time
                   } else if(method == "cwt"){
                          framework$IndividualIndices[[t_locator]]$CWT <- results
                          framework$IndividualIndices[[t_locator]]$Time_CWT <- Time
                   }
                   return(framework)
}
...
AddExpectedHRV <- function(framework, a_locator, t_locator, use.name = TRUE){
  interval <- framework$IndividualIndices[[t_locator]]
  others <- interval$HRV
  ...
  time_flags <- framework$IndividualIndices[[t_locator]]$Time_DWT
  hrv <- Analysis$HRV
  ...
  if(N == 0){
    ...
    results <- others
    results[,a_locator] <- hrv
    hrv <- results
  } else {
    hrv <- cbind(others, hrv)
  }
  if(use.name) colnames(hrv)[a_locator] <- Names[a_locator]
  framework$IndividualIndices[[t_locator]]$HRV <- hrv
  return(framework)
}
...
AddTimeValues <- function(framework, a_locator, t_locator, time_flags,
  use.name = TRUE){
                   interval <- framework$IndividualIndices[[t_locator]]
                   Analysis <- framework$Analyses[[a_locator]]
                   HR <- interval$HR
                   ...
                   } else if(a_locator <= ncol(HR)){
                     HR[,a_locator] <- TimeValues$HR
                     SBP[,a_locator] <- TimeValues$SBP
                   } else {
                      HR <- cbind(HR, TimeValues$HR)
                      SBP <- cbind(SBP, TimeValues$SBP)
                   }
                   if(use.name & !is.null(Analysis$Name)){
                         colnames(HR)[a_locator] <- Analysis$Name
                         colnames(SBP)[a_locator] <- Analysis$Name
                   } else {
                         colnames(HR)[a_locator] <- Analysis$locator
                         colnames(SBP)[a_locator] <- Analysis$locator
                   }
                   framework$IndividualIndices[[t_locator]]$HR <- HR
                   framework$IndividualIndices[[t_locator]]$SBP <- SBP
                   return(framework)
}
