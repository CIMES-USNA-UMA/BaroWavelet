...
#' @examples
#' Data <- DataSimulation()
DataSimulation <- function(use.noise = TRUE, seed = 1, v = 9, s = c(0.25, 1.5, 2.75, 2, 0.75)) 
{
  set.seed(seed)
  ...
  SBP <- 5 * sin(2 * pi * 0.1 * time) + 5 * sin(2 * pi * 
                                                  0.25 * time) + use.noise * rnorm(2500, mean = 0, 
                                                                                   sd = 2)
  RR <- c(rep(v, 500), rep(0, 500), rep(v, 500), rep(0, 500), rep(v, 500)) * SBP
  for (n in c(6:1000, 2006:2500)) RR[n] <- RR[n] + sum(SBP[(n - 1):(n - 5)] * s)
  set.seed(seed + 1)
