...
AlphaIndexDWT <- function(data, HF = 0.4, LF = 0.15, VLF = 0.04, 
                  ...
                  RHRVobjects <- SendDataToRHRV(RHRVobjects, HF, LF, VLF, wv, error,
                                                error.type)
...
SendDataToRHRV <- function(RHRVobjects, HF = 0.4, LF = 0.15, VLF = 0.04,
                           wv = "d4", error = 0.01, error.type = "absolute"){
  for(n in 1:2){
    RHRVobjects[[n]] <-
    ...
    RHRVobjects[[n]] <- RHRV::CalculatePowerBand(
      RHRVobjects[[n]], ULFmax = 0.003, VLFmin = 0.003,
      VLFmax = VLF, LFmin = VLF,
      LFmax = LF,
      HFmin = LF, HFmax = HF,
      type = "wavelet", wavelet = wv, bandtolerance = error, 
